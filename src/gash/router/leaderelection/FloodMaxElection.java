...
import gash.router.leaderelection.Message.Delivery;
import gash.router.leaderelection.Message.Direction;
...
public class FloodMaxElection {
	...
	public FloodMaxElection(int numNodes) throws Exception {
	...
	public FloodMaxElection(){
		transport = new Ring(0);
		Enumeration<Integer> keys = NodeChannelManager.node2ChannelMap.keys();
		while(keys.hasMoreElements()){
			Integer nodeId = keys.nextElement();
			FloodNode fn = new FloodNode(nodeId);
			fn.start();
	...
	public static class FloodNode extends RingNode {
		...
		private void sendElectionNotice(FloodMsg originalMsg) {
			if (originalMsg == null) {
				FloodMsg msg = new FloodMsg(FloodMaxElection.msgID.incrementAndGet());
